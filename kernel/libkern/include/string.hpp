...
#define NOSTD_STRING_CONTAINERS_RANGES 1
	...
	(__cplusplus <= 202002L || !__has_include(<ranges>) || !defined(__cpp_lib_containers_ranges))
...
#include <stdexcept>
#define _NOSTD_STRING_ASSERT(x, str, e) \
	do {                            \
		if (!(x))               \
			throw e(str);   \
	} while (0)
...
#else
#define _NOSTD_STRING_ASSERT(x, str, ...)                          \
	do {                                                       \
		if (!(x)) {                                        \
			NOSTD_STRING_FALLBACK_ABORT_FUNCTION(str); \
			{                                          \
				while (true) {                     \
					[] {}();                   \
				}                                  \
...
#define _NOSTD_STRING_DIAG_PUSH() \
	_NOSTD_STRING_PRAGMA(_NOSTD_STRING_PRAGMA_DIAG_PREFIX diagnostic push)
#define _NOSTD_STRING_DIAG_IGN(wrn) \
	_NOSTD_STRING_PRAGMA(       \
		_NOSTD_STRING_PRAGMA_DIAG_PREFIX diagnostic ignored wrn)
...
struct is_allocator<Allocator,
		    std::void_t<typename Allocator::value_type,
				decltype(std::declval<Allocator &>().allocate(
...
concept container_compatible_range =
	...
	std::convertible_to<std::ranges::range_reference_t<Range>, Type>;
#endif
} // namespace detail
...
class basic_string {
	...
	{
		if (auto &buffer = this->storage._long.data;
		    this->is_long() && buffer != nullptr) {
			this->_allocator.deallocate(
				buffer, this->storage._long.cap + 1);
			buffer = nullptr;
		}
	...
	{
		return this->is_long() ? this->storage._long.data :
					 this->storage._short.data;
	}
	...
	{
		return this->is_long() ? this->storage._long.size :
					 this->storage._short.size;
	}
	...
	constexpr bool addr_in_range(const_pointer ptr) const
	{
		if (std::is_constant_evaluated())
			return false;
		return this->get_data() <= ptr &&
		       ptr <= this->get_data() + this->get_size();
	}
					     ...
					     size_type count)
	{
		...
		if (oldcount != count)
			Traits::move(this->get_data() + pos + count,
				     this->get_data() + pos + oldcount,
				     sz - pos - oldcount);
					...
					size_type oldcount, size_type count)
	{
		if (this->addr_in_range(str)) {
			...
			this->internal_replace_impl(
				[&]() {
					Traits::copy(this->get_data() + pos,
						     rstr.data(), count);
				},
				pos, oldcount, count);
		} else
					...
					size_type oldcount, size_type count)
	{
		this->internal_replace_impl(
			[&]() {
				Traits::assign(this->get_data() + pos, count,
					       ch);
			},
			pos, oldcount, count);
	}
					    ...
					    size_type size)
	{
		if (size != 0) {
			...
			Traits::move(this->get_data() + pos + size,
				     this->get_data() + pos, sz - pos);
			func();
	...
	{
		this->internal_assign_impl(
			[&](auto data) { Traits::assign(data, count, ch); },
			count, copy_old);
	}
	...
	explicit constexpr basic_string(const allocator_type &a) noexcept
		: _allocator(a)
	{
		this->short_init();
	}
		...
		: _allocator(a)
	{
		_NOSTD_STRING_ASSERT(
			count <= this->max_size(),
			"nostd::basic_string::basic_string(): constructed string size would exceed max_size()",
			std::length_error);
		...
		: _allocator(a)
	{
		_NOSTD_STRING_ASSERT(
			pos <= str.get_size(),
			"nostd::basic_string::basic_string(): pos out of range",
			std::out_of_range);
	...
	requires(std::is_convertible_v<const Type &, sview_type>) constexpr
		basic_string(const Type &t, size_type pos, size_type count,
			     const allocator_type &a = allocator_type())
		: _allocator(a)
	{
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &,
					const Char *>) explicit constexpr
		basic_string(const Type &t,
			     const allocator_type &a = allocator_type())
		: _allocator(a)
	{
		sview_type sv(t);
	...
	constexpr basic_string(std::from_range_t, Range &&range,
			       const Allocator &a = Allocator())
		: basic_string(std::ranges::begin(range),
			       std::ranges::end(range), a)
	{
	}
#endif
	...
	template <typename Type>
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>) constexpr
		basic_string &
	...
	constexpr basic_string &assign(size_type count, value_type ch)
	{
		_NOSTD_STRING_ASSERT(
			count <= this->max_size(),
			"nostd::basic_string::basic_string(): resulted string size would exceed max_size()",
			std::length_error);
		this->internal_assign(ch, count);
		return *this;
	}
				       ...
				       size_type count = npos)
	{
		_NOSTD_STRING_ASSERT(
			pos <= str.get_size(),
			"nostd::basic_string::assign(): pos out of range",
			std::out_of_range);
	...
	constexpr basic_string &assign(const value_type *str, size_type count)
	{
		_NOSTD_STRING_ASSERT(
			count <= this->max_size(),
			"nostd::basic_string::assign(): resulted string size would exceed max_size()",
			std::length_error);
		this->internal_assign(str, count);
		return *this;
	}
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>) constexpr
		basic_string &assign(const Type &t)
	{
		sview_type sv(t);
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>) constexpr
		basic_string &assign(const Type &t, size_type pos,
				     size_type count = npos)
	{
	...
	{
		auto str = basic_string(std::from_range,
					std::forward<Range>(range),
					this->_allocator);
		_NOSTD_STRING_ASSERT(
			str.get_size() <= this->max_size(),
			"nostd::basic_string::assign_range(): resulted string size would exceed max_size()",
			std::length_error);
		return this->assign();
	}
#endif
	...
	constexpr reference at(size_type pos)
	{
		_NOSTD_STRING_ASSERT(
			pos < this->get_size(),
			"nostd::basic_string::at(): pos out of range",
			std::out_of_range);
		return this->get_data()[pos];
	}
	...
	constexpr void reserve(size_type cap)
	{
		_NOSTD_STRING_ASSERT(
			cap <= this->max_size(),
			"nostd::basic_string::reserve(): allocated memory size would exceed max_size()",
			std::length_error);
		if (cap <= this->get_cap())
			return;
				       ...
				       value_type ch)
	{
		_NOSTD_STRING_ASSERT(
			...
			"nostd::basic_string::insert(): resulted string size would exceed max_size()",
			std::length_error);
		_NOSTD_STRING_ASSERT(
			pos <= this->get_size(),
			"nostd::basic_string::insert(): pos out of range",
			std::out_of_range);
				       ...
				       size_type count = npos)
	{
		_NOSTD_STRING_ASSERT(
			...
			"nostd::basic_string::insert(): pos or pos_str out of range",
			std::out_of_range);
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>) constexpr
		basic_string &insert(size_type pos, const Type &t)
	{
		_NOSTD_STRING_ASSERT(
			pos <= this->get_size(),
			"nostd::basic_string::insert(): pos out of range",
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>) constexpr
		basic_string &insert(size_type pos, const Type &t,
				     size_type pos_str, size_type count = npos)
	{
		auto sv = sview_type(t);
		_NOSTD_STRING_ASSERT(
	...
	constexpr iterator insert_range(const_iterator pos, Range &&range)
	{
		...
		_NOSTD_STRING_ASSERT(
			this->get_size() + str.get_size() <= this->max_size(),
			"nostd::basic_string::insert_range(): resulted string size would exceed max_size()",
			std::length_error);
	...
	constexpr basic_string &erase(size_type pos = 0, size_type count = npos)
	{
		...
		_NOSTD_STRING_ASSERT(
			pos <= sz,
			"nostd::basic_string::erase(): pos out of range",
			std::out_of_range);
	...
	constexpr void push_back(value_type ch)
	{
		_NOSTD_STRING_ASSERT(
			...
			"nostd::basic_string::push_back(): resulted string size would exceed max_size()",
			std::length_error);
		this->append(1, ch);
	}
	...
	constexpr basic_string &append(size_type count, value_type ch)
	{
		_NOSTD_STRING_ASSERT(
			this->get_size() + count <= this->max_size(),
			"nostd::basic_string::append(): resulted string size would exceed max_size()",
			std::length_error);
				       ...
				       size_type count = npos)
	{
		_NOSTD_STRING_ASSERT(
			pos <= str.get_size(),
			"nostd::basic_string::append(): pos out of range",
			std::out_of_range);
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>) constexpr
		basic_string &append(const Type &t)
	{
		sview_type sv(t);
		_NOSTD_STRING_ASSERT(
			this->get_size() + sv.length() <= this->max_size(),
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>) constexpr
		basic_string &append(const Type &t, size_type pos,
				     size_type count = npos)
	{
		sview_type sv(t);
		_NOSTD_STRING_ASSERT(
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>) constexpr
		int compare(const Type &t) const noexcept(noexcept(
			std::is_nothrow_convertible_v<const Type &, sview_type>))
	{
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>) constexpr
		int compare(size_type pos1, size_type count1, const Type &t,
					...
					const basic_string &str)
	{
		_NOSTD_STRING_ASSERT(
			pos <= this->get_size(),
			"nostd::basic_string::replace(): pos out of range",
			std::out_of_range);
					...
					size_type count2 = npos)
	{
		_NOSTD_STRING_ASSERT(
			...
			"nostd::basic_string::replace(): pos or pos_str out of range",
			std::out_of_range);
	...
	replace(const_iterator first, const_iterator last, InputIterator first2,
		InputIterator last2)
	{
		return this->replace(first, last, const_pointer(first2),
					...
					const value_type *str, size_type count2)
	{
		...
		_NOSTD_STRING_ASSERT(
			...
			"nostd::basic_string::replace(): resulted string size would exceed max_size()",
			std::length_error);
	...
	constexpr basic_string &replace(const_iterator first,
					...
					std::initializer_list<value_type> ilist)
	{
		return this->replace(first, last, const_pointer(ilist.begin()),
				     ilist.size());
	}
...
#if NOSTD_STRING_CONTAINERS_RANGES
	template <detail::container_compatible_range<Char> Range>
	constexpr iterator replace_with_range(const_iterator first,
					      const_iterator last,
					      Range &&range)
	{
		auto str = basic_string(std::from_range,
					std::forward<Range>(range),
					this->_allocator);
		return this->replace(first, last,
				     str); // replace checks for max_size()
	}
#endif
				      ...
				      size_type count = npos) const
	{
		_NOSTD_STRING_ASSERT(
			pos <= this->get_size(),
			"nostd::basic_string::substr(): pos out of range",
			std::out_of_range);
				 ...
				 size_type pos = 0) const
	{
		_NOSTD_STRING_ASSERT(
			pos <= this->get_size(),
			"nostd::basic_string::copy(): pos out of range",
			std::out_of_range);
	...
	constexpr void resize(size_type count, value_type ch)
	{
		_NOSTD_STRING_ASSERT(
			this->get_size() + count <= this->max_size(),
			"nostd::basic_string::resize(): resulted string size would exceed max_size()",
			std::length_error);
		auto cap = this->get_cap();
		auto sz = this->get_size();
		auto rsz = count + sz;
	...
	constexpr void resize_and_overwrite(size_type, Operation)
	{
		static_assert(
			...
			"nostd::basic_string::resize_and_overwrite(count, op) not implemented!");
	}
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>) constexpr
		size_type rfind(const Type &t, size_type pos = npos) const
		noexcept(std::is_nothrow_convertible_v<const Type &, sview_type>)
	{
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>) constexpr
		size_type find_first_of(const Type &t, size_type pos = 0) const
		noexcept(std::is_nothrow_convertible_v<const Type &, sview_type>)
	{
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>)
		constexpr size_type
		find_first_not_of(const Type &t, size_type pos = 0) const
		noexcept(std::is_nothrow_convertible_v<const Type &, sview_type>)
	{
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>)
		constexpr size_type
		find_last_of(const Type &t, size_type pos = npos) const
		noexcept(std::is_nothrow_convertible_v<const Type &, sview_type>)
	{
	...
	constexpr size_type find_last_not_of(
		const basic_string &str, size_type pos = npos) const noexcept
	{
		return this->get_view().find_last_not_of(sview_type(str), pos);
	}
	...
	constexpr size_type find_last_not_of(
		const value_type *str, size_type pos = npos) const noexcept
	{
		return this->get_view().find_last_not_of(str, pos);
	}
	...
	requires(std::is_convertible_v<const Type &, sview_type> &&
		 !std::is_convertible_v<const Type &, const Char *>)
		constexpr size_type
		find_last_not_of(const Type &t, size_type pos = npos) const
		noexcept(std::is_nothrow_convertible_v<const Type &, sview_type>)
	{
						...
						const basic_string &rhs)
	{
		...
		basic_string ret(detail::uninitialized_size_tag(),
				 lhs_sz + rhs_sz,
				 basic_string::alloc_traits::
	...
	friend constexpr basic_string operator+(const Char *lhs,
						const basic_string &rhs)
	{
	...
	friend constexpr basic_string operator+(Char lhs,
						const basic_string &rhs)
	{
		auto rhs_sz = rhs.size();
		basic_string ret(detail::uninitialized_size_tag(), rhs_sz + 1,
				 basic_string::alloc_traits::
					 select_on_container_copy_construction(
						 rhs.get_allocator()));
		auto buffer = ret.get_data();
	...
	{
		auto lhs_sz = lhs.size();
		basic_string ret(detail::uninitialized_size_tag(), lhs_sz + 1,
				 basic_string::alloc_traits::
					 select_on_container_copy_construction(
						 lhs.get_allocator()));
		auto buffer = ret.get_data();
...
template <typename Char, typename Traits, typename Allocator>
constexpr bool operator==(const basic_string<Char, Traits, Allocator> &lhs,
			  const Char *rhs)
{
	return lhs.compare(rhs) == 0;
}
...
template <typename Char, typename Traits, typename Allocator>
	constexpr std::strong_ordering operator<= >
	(const basic_string<Char, Traits, Allocator> &lhs, const Char *rhs)
{
	return lhs.compare(rhs) <= > 0;
}
#endif
...
// swap
template <typename Char, typename Traits, typename Allocator>
constexpr void swap(basic_string<Char, Traits, Allocator> &lhs,
...
constexpr typename basic_string<Char, Traits, Allocator>::size_type
erase(basic_string<Char, Traits, Allocator> &c, const U &value)
{
...
constexpr typename basic_string<Char, Traits, Allocator>::size_type
erase_if(basic_string<Char, Traits, Allocator> &c, Pred pred)
{
...
basic_string(InputIt, InputIt, Allocator = Allocator())
	-> basic_string<typename std::iterator_traits<InputIt>::value_type,
			std::char_traits<typename std::iterator_traits<
				InputIt>::value_type>,
			Allocator>;

template <typename Char, typename Traits,
	  typename Allocator = std::allocator<Char> >
explicit basic_string(std::basic_string_view<Char, Traits>,
		      const Allocator & = Allocator())
	-> basic_string<Char, Traits, Allocator>;

template <typename Char, typename Traits,
	  typename Allocator = std::allocator<Char> >
basic_string(std::basic_string_view<Char, Traits>,
...
template <std::ranges::input_range Range,
	  typename Allocator =
		  std::allocator<std::ranges::range_value_t<Range> > >
basic_string(std::from_range_t, Range &&, Allocator = Allocator())
	-> basic_string<std::ranges::range_value_t<Range>,
...
inline long long stoll(const string &str, std::size_t *pos = nullptr,
		       int base = 10)
{
	auto cstr = str.c_str();
	char *ptr = const_cast<char *>(cstr);
...
inline unsigned long stoul(const string &str, std::size_t *pos = nullptr,
			   int base = 10)
{
	auto cstr = str.c_str();
	char *ptr = const_cast<char *>(cstr);
...
inline unsigned long long stoull(const string &str, std::size_t *pos = nullptr,
				 int base = 10)
{
	auto cstr = str.c_str();
	char *ptr = const_cast<char *>(cstr);
...
namespace detail
{
template <typename Type> constexpr std::size_t to_chars_len(Type value)
{
...
template <std::signed_integral Type,
	  std::unsigned_integral UType = std::make_unsigned_t<Type> >
[[gnu::always_inline]]
constexpr inline string to_string(Type value)
{
	const auto negative = value < 0;
	const UType uvalue =
		negative ? static_cast<UType>(~value) + static_cast<UType>(1) :
			   value;
...
constexpr uint64_t MurmurHash2_64A(const void *key, uint64_t len, uint64_t seed)
{
	...
	auto data2 =
		static_cast<const uint8_t *>(static_cast<const void *>(data));
...
template <typename Char, typename Allocator,
	  typename String =
		  basic_string<Char, std::char_traits<Char>, Allocator> >
struct string_hash_base {
	[[nodiscard]] constexpr std::size_t
	operator()(const String &str) const noexcept
	{
		return MurmurHash2_64A(str.c_str(), str.length() * sizeof(Char),
				       0xC70F6907UL);
	}
};
} // namespace detail
...
constexpr inline string operator"" s(const char *str, std::size_t len)
{
	return string{ str, len };
}
constexpr inline u8string operator"" s(const char8_t *str, std::size_t len)
{
	return u8string{ str, len };
}
constexpr inline u16string operator"" s(const char16_t *str, std::size_t len)
{
	return u16string{ str, len };
}
constexpr inline u32string operator"" s(const char32_t *str, std::size_t len)
{
	return u32string{ str, len };
}
constexpr inline wstring operator"" s(const wchar_t *str, std::size_t len)
{
...
{
template <typename Allocator>
struct hash<nostd::basic_string<char, std::char_traits<char>, Allocator> >
	: nostd::detail::string_hash_base<char, Allocator> {};

template <typename Allocator>
struct hash<nostd::basic_string<char8_t, std::char_traits<char8_t>, Allocator> >
	: nostd::detail::string_hash_base<char8_t, Allocator> {};

template <typename Allocator>
struct hash<nostd::basic_string<char16_t, std::char_traits<char16_t>, Allocator> >
	: nostd::detail::string_hash_base<char16_t, Allocator> {};

template <typename Allocator>
struct hash<nostd::basic_string<char32_t, std::char_traits<char32_t>, Allocator> >
	: nostd::detail::string_hash_base<char32_t, Allocator> {};

template <typename Allocator>
struct hash<nostd::basic_string<wchar_t, std::char_traits<wchar_t>, Allocator> >
	: nostd::detail::string_hash_base<wchar_t, Allocator> {};
} // namespace std
#endif
