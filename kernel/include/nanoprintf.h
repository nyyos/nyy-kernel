...
static int npf_utoa_rev(char *buf, npf_uint_t i, unsigned base,
			unsigned case_adjust);

#if NANOPRINTF_USE_FLOAT_FORMAT_SPECIFIERS == 1
static int npf_fsplit_abs(float f, uint64_t *out_int_part,
			  uint64_t *out_frac_part, int *out_frac_base10_neg_e);
static int npf_ftoa_rev(char *buf, float f, npf_format_spec_t const *spec,
			int *out_frac_chars);
#endif
		   ...
		   int *out_frac_base10_neg_exp)
{
	...
	{ // Convert the fractional part to base 10.
		...
		for (int i = 0; frac && (i < NPF_MAX_FRACTION_DEC_DIGITS);
		     ++i) {
			frac_part *= 10;
			frac_part += (uint64_t)(frac >>
						(NPF_FRACTION_BIN_DIGITS - 4));
			frac &= 0x0fffffffffffffffllu;
			frac *= 10;
		}
...
int npf_ftoa_rev(char *buf, float f, npf_format_spec_t const *spec,
		 int *out_frac_chars)
{
	uint32_t f_bits;
	{ // union-cast is UB, let compiler optimize byte-copy loop.
	...
	if ((uint8_t)(f_bits >> 23) == 0xFF) {
		if (f_bits & 0x7fffff) {
			for (int i = 0; i < 3; ++i) {
				*buf++ = (char)("NAN"[i] + spec->case_adjust);
			}
		} else {
			for (int i = 0; i < 3; ++i) {
				*buf++ = (char)("FNI"[i] + spec->case_adjust);
			}
		}
		return -3;
	}
	...
	if (npf_fsplit_abs(f, &int_part, &frac_part, &frac_base10_neg_exp) ==
	    0) {
		for (int i = 0; i < 3; ++i) {
			*buf++ = (char)("ROO"[i] + spec->case_adjust);
		}
		return -3;
	}
...
int npf_vpprintf(npf_putc pc, void *pc_ctx, char const *format, va_list args)
{
	...
	while (*cur) {
		int const fs_len =
			(*cur != '%') ? 0 : npf_parse_format_spec(cur, &fs);
		if (!fs_len) {
			NPF_PUTC(*cur++);
			continue;
		}
		...
		case NPF_FMT_SPEC_CONV_SIGNED_INT: {
			...
			if (!val && (fs.prec_opt == NPF_FMT_SPEC_OPT_LITERAL) &&
			    !fs.prec) {
				cbuf_len = 0;
			} else
#endif
		...
		case NPF_FMT_SPEC_CONV_UNSIGNED_INT: {
			...
			if (!val && (fs.prec_opt == NPF_FMT_SPEC_OPT_LITERAL) &&
				...
				if ((fs.conv_spec == NPF_FMT_SPEC_CONV_OCTAL) &&
				    fs.alt_form) {
					fs.prec =
						1; // octal special case, print a single '0'
				}
			} else
#endif
#if NANOPRINTF_USE_BINARY_FORMAT_SPECIFIERS == 1
					 ...
					 NPF_FMT_SPEC_CONV_OCTAL) ?
						...
						((fs.conv_spec ==
						  NPF_FMT_SPEC_CONV_HEX_INT) ?
							 ...
							 10u);
				cbuf_len =
					npf_utoa_rev(cbuf, val, base,
						     (unsigned)fs.case_adjust);
			}

			if (val && fs.alt_form &&
			    (fs.conv_spec == NPF_FMT_SPEC_CONV_OCTAL)) {
				cbuf[cbuf_len++] =
					'0'; // OK to add leading octal '0' immediately.
			}

			if (val &&
			    fs.alt_form) { // 0x or 0b but can't write it yet.
		...
		case NPF_FMT_SPEC_CONV_POINTER: {
			cbuf_len = npf_utoa_rev(
				cbuf,
				(npf_uint_t)(uintptr_t)va_arg(args, void *), 16,
				'a' - 'A');
			need_0x = 'x';
		} break;
...
#if NANOPRINTF_USE_FLOAT_FORMAT_SPECIFIERS == 1
		if ((fs.conv_spec == NPF_FMT_SPEC_CONV_FLOAT_DEC) && !fs.prec &&
		    !fs.alt_form) {
		...
		if (!fs.left_justified &&
		    pad_c) { // If leading zeros pad, sign goes first.
			if (pad_c == '0') {
				if (sign_c) {
					NPF_PUTC(sign_c);
					sign_c = 0;
