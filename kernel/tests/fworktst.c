...
NO_RETURN void T_Particle(void *Parameter, UNUSED void *)
{
...
NO_RETURN void T_Explodeable(UNUSED void *Parameter, UNUSED void *Parameter2)
{
	timer_t Timer;
	timer_initialize(&Timer, 0, nullptr, 0);
...
void SpawnParticle(PFIREWORK_DATA Data)
{
	...
	sched_init_thread(Thrd, T_Particle, kPriorityHigh, Data, nullptr);
	sched_resume(Thrd);
	/*
	if (!Thrd)
		MmFreePool(Data);
...
{
	thread_t *Thrd = sched_alloc_thread();
	sched_init_thread(Thrd, T_Explodeable, kPriorityHigh, nullptr, nullptr);
	sched_resume(Thrd);
	/*
	if (Thrd)
		ObDereferenceObject(Thrd);
...
void PerformFireworksTest()
{
	...
	//printk("\x1B[40m\x1B[1;1HThe Boron Operating System - Fireworks test\nHappy New Year 2024!");
	...
	while (true) {
		...
		sched_sleep(MS2NS(2000 + Rand() % 2000));
	}
}
