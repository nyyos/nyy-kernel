...
mkdir -p "${ISOROOT}" &&
	cp build-${PORT}/platform/amd64/symbols.map build-${PORT}/platform/amd64/nyy ${LIMINE}/limine-bios.sys ${LIMINE}/limine-bios-cd.bin ${LIMINE}/limine-uefi-cd.bin "${ISOROOT}" &&
